...
### Fixed
- **Per-request event streams are sent as `text/event-stream`**: `openEventStream()` set the response headers and then replaced the body with a `NonBufferedBody`. Slim registers a header with PHP at the moment it is set only when the body is already unbuffered, so every header set before the swap was lost and a `tools/call`, `prompts/get` or `resources/read` carrying a `progressToken` or a log level streamed its events under PHP's default `text/html`. Clients refused the response, and Claude Code, which sends a progress token on every call, could not call a tool at all under 2026-07-28. The body is now swapped first, as the GET stream transports already did.
